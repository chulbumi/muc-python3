...
Rust/Python 엔진과 Rhai 명령은 이 공개 저장소에 있으며, 실제 맵·몹·아이템·이벤트
데이터는 private `chulbumi/muc-python3-data` 저장소가 `data/` submodule로
제공합니다. 데이터 저장소 권한이 있는 운영자는 다음과 같이 받습니다.

```bash
git clone --recurse-submodules https://github.com/chulbumi/muc-python3.git
cd muc-python3
./scripts/init-runtime-data.sh
```

이미 코드만 clone했다면 `git submodule update --init data`를 실행합니다. 권한이
없는 공개 기여자는 `test/fixtures/public-data/`의 합성 데이터로 로더와 엔진을
검증할 수 있지만 실제 게임 월드는 실행할 수 없습니다.

사용자 저장, 로그, Soul, 개인 보관함, 길드·순위·단일아이템 상태는 Git에 넣지 않고
`runtime/`에 저장합니다. 서버 시작 시 누락된 디렉터리와 초기 상태가 자동으로
준비되며, 외부 볼륨을 쓸 때는 `MUC_RUNTIME_DIR`을 지정할 수 있습니다.
...
- `data/` — private 맵·몹·아이템·이벤트 submodule
- `runtime/` — Git에서 제외되는 사용자·로그·가변 서버 상태
- `test/fixtures/public-data/` — 공개 로더 검증용 합성 데이터
- `skills/mud-test/` — Python/Rust raw socket 비교 도구

## CI/CD

GitHub Actions에서 공개 코드 검사는 private 데이터 없이 수행합니다. 같은 저장소의
push와 내부 PR은 read-only deploy key로 private submodule을 받아 전체 테스트를
추가로 수행합니다. 외부 fork에는 해당 key가 전달되지 않습니다.

- `cargo fmt --check`
- `cargo clippy`
- 공개 합성 데이터 로더 테스트
- 내부 변경의 private 데이터 전체 테스트
