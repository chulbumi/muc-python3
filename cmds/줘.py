...
class CmdObj(Command):
    ...
    def cmd(self, ob, line):
        ...
        if target == None or not is_player(target):
            ...
            return
        if target == ob:
            ob.sendLine('당신이 [36m' + post + '[37m 가지고 장난합니다. \'@_@\'')
            return
        i = 1
        c = 0
        if len(words) >= 3:
            ...
            if target.getItemWeight() + obj['무게'] > target.getStr() * 10:
                if c == 0:
                    ...
                    target.sendLine('\r\n[1m' + ob['이름'] + '[0;37m' + han_iga(ob['이름']) + ' 줄려는 ' + 
                        '[36m' + obj.han_obj() + '[37m 무거워서 받지 못합니다.')
                    target.lpPrompt()
                    return
                break
            if target.getItemCount() >= getInt(MAIN_CONFIG['사용자아이템갯수']):
                if c == 0:
                    ob.sendLine('[1m' + target['이름'] + '[0;37m' + han_iga(target['이름']) + \
                        ' 수량 한계로 받지 못합니다.')
                    target.sendLine('\r\n[1m' + ob['이름'] + '[0;37m' + han_iga(ob['이름']) + ' 줄려는 ' + \
                        '[36m' + obj.han_obj() + '[37m 수량 한계로 받지 못합니다.')
                    target.lpPrompt()
                    return
                break
            c += 1
        ...
        if c == 0:
            ...
            ob.sendLine('당신이 [1m' + target['이름'] + '[0;37m에게 [36m' + post + '[37m 줍니다.')
            target.sendLine('\r\n[1m' + ob['이름'] + '[0;37m' + han_iga(ob['이름']) + ' 당신에게 [36m' + post + '[37m 줍니다.')
            ob.sendRoom('%s %s에게 [36m%s[37m 줍니다.' % ( ob.han_iga(), target.getNameA(), post), ex = target)
            target.lpPrompt()
        else:
