...
pub use processor::{
    CombatAction, CombatRound,
    calculate_player_damage, calculate_mob_damage,
    check_hit, process_player_attack, start_combat, find_mob_in_room,
};
...
pub fn attack_mob_command(
    ...
    // Find mob in room
    let (mob_instance, mob_data) = match processor::find_mob_in_room(&player.get_name(), target_name, world) {
        Some(result) => result,
        None => {
            // Try to find player instead (PvP)
            let player_pos = match world.get_player_position(&player.get_name()) {
                Some(pos) => pos,
                None => return CommandResult::Error("☞ 당신의 위치를 찾을 수 없습니다.".to_string()),
            };
