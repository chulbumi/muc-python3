...
class ClientThread(threading.Thread):
    ...
    def run(self):
        ...
        while not terminate and not terminateAll:
            ...
            try:
                inputsReady, outputsReady, errorsReady = select.select(
                    inputs, outputs, [], 1.0
                )
            except Exception, e:
                print e
            ...
            for inp in inputsReady:
                if inp == self.__clientSocket:
                    try:
                        data = self.__clientSocket.recv(40960)
                        #data = data.decode("utf-8").encode("euc-kr")
                    ...
                    try:
                        data = targetHostSocket.recv(40960)
                        #data = data.decode("euc-kr").encode("utf-8")
                        data = data.decode("utf-8").encode("euc-kr")
                    except Exception, e:
                        print e
